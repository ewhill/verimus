...
Do **not** use `as any` casting unless monkey-patching poorly defined external dependency types (e.g., overriding `p2p` connections). This restriction applies to both production logic and unit test mock generation. Do not construct mock objects using `as any` or `as unknown as T`. Instead, use explicit typing to strictly map interface boundaries. 
...
- **Test Coverage**: Every explicit change must include updates to the corresponding unit or integration tests. If current coverage is inadequate, you must add new test cases. An agent should **NEVER** submit code in a git commit that includes failing tests. All tests must be run (`npm test`) prior to making a commit and ensured to be passing fully.
...
- **ESLint Correction**: Directly after passing tests and syntax bounds, you **must** execute `npx eslint --fix "<path_to_modified_files>"` (e.g., `npx eslint --fix "bundler/**/*.ts"`) to format and restrict your import patterns properly. Never execute a git commit without confirming 0 linting errors on your touched files.
- **Strict Mock Integrity**: Do not pollute production source code with defensive logic (e.g., `typeof method === 'function'` checks) merely to appease incomplete mocks or bypass failing unit tests. When building test mocks, utilize `node:test`'s `mock.fn()` utilities and precise `createMock<T>({})` intercept mappings to strictly maintain type safety and allow for robust mock function call verification. Note: `createMock` should be explicitly imported from the central `test/utils/TestUtils.ts` file rather than redefined inline inside individual tests. Additionally, `mock.fn()` must always include explicit sub-typing that matches the mocked function's signature (e.g., mocking `bar(): Promise<string>` must strictly be executed as `mock.fn<() => Promise<string>>()`). Mocks must reflect the structural contract of their production counterparts without resorting to weak bounds or bypasses. If a handler legitimately expects an interface method, identify and update the test-harness payload structs to fulfill that architectural contract. Production files must stay strictly restricted to runtime logic.
