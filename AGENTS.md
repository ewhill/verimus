...
- **Strict Mock Integrity**: Do not pollute production source code with defensive logic (e.g., `typeof method === 'function'` checks) merely to appease incomplete mocks or bypass failing unit tests. If a handler legitimately expects an interface method, you must identify and update the test-harness payload structs to fulfill that architectural contract. Production files must stay purely restricted to runtime logic.
