...
- **Insufficient Base Collateral:** If a node attempts to bid on a file contract but their `WalletManager` locked collateral state is below the minimum threshold, the protocol must reject their bid payload.

## 7. Granular Engineering Task Breakdown
1. Augment `WalletManager.calculateBalance()` to track base collateral deposits, preventing `allocateFunds()` overdraw scenarios.
...
### Cons
- Attackers deduce profitability margins, balancing dumps to neutralize deterrence structures.
