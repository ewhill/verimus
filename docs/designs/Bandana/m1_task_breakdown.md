...
- Add `expirationBlockHeight?: number;` to track the expiration epoch.
...
- For each escrow, calculate the base payout ratio: `payoutWei = escrow.allocatedRestToll / (escrow.expirationBlockHeight - escrow.startBlockHeight)`.
