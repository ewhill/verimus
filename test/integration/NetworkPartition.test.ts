...
describe('Integration: Network Partition Resiliency & Byzantine Fault Simulation (Phase 3)', () => {
    ...
    it('Drops minority forks during Network Partition', async () => {
        ...
        node3.mempool.pendingBlocks.set('rogue_block', {
            block: { type: 'TRANSACTION', metadata: { index: 1, timestamp: 1 }, signerAddress: 'pk', signature: 'sig', hash: 'rogue_block', payload: { senderSignature: '', senderAddress: '', recipientAddress: '', amount: 0n } },
    ...
    it('Coordinates adoption across majority segments', async () => {
        node1.mempool.pendingBlocks.set('valid_block', {
            block: { type: 'TRANSACTION', metadata: { index: 2, timestamp: 2 }, signerAddress: 'pk', signature: 'sig', hash: 'valid_block', payload: { senderSignature: '', senderAddress: '', recipientAddress: '', amount: 0n } },
            verifications: new Set(),
            eligible: true,
            originalTimestamp: Date.now()
        } as any);
