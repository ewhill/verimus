...
async function runTest() {
    ...
    const uploadRes = await uploadBlock(26780, filename, '# Test Search File\nTesting search');
    ...
    let exactFound = searchExact.blocks.some(b => b.hash === hash);
    if (!exactFound) {
    ...
    let partialFound = searchPartial.blocks.some(b => b.hash === hash);
    if (!partialFound) {
    ...
    let invalidFound = searchInvalid.blocks.some(b => b.hash === hash);
    if (invalidFound) {
        console.error("ERROR: Block incorrectly returned when searching for an invalid filename.", searchInvalid.blocks);
        process.exit(1);
    } else {
