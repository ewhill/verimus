...
test("Message", () => {
  ...
  assert.strictEqual(JSON.stringify(emptyMessage.body), "{}",
  ...
  assert.deepStrictEqual(messageWithTypeAndBody.body, messageBody,
  ...
  assert.notStrictEqual(messageWithTypeAndBody.header.hash, messageHeaderHashBefore,
  ...
  assert.notStrictEqual(messageWithTypeAndBody.header.hash, messageHeaderHashChanged,
  ...
  assert.notStrictEqual(messageWithTypeAndBody.header.hash, messageHeaderHashUpdated,
  ...
  assert.strictEqual(simpleStringMessage.body.greeting, "hello world!",
  ...
  assert.deepStrictEqual(simpleStringMessage, clonedMessage,
    "Generated and cloned should be equal.");

  // firstMessage and secondMessage should have the same toString values
  assert.strictEqual(simpleStringMessage.toString(), clonedMessage.toString(),
  ...
  assert.ok(blankMessage.header.timestamp instanceof Date,
    "Generated header timestamp should be an instance of Date object.");

  blankMessage = Message.from(simpleStringMessage);

  assert.strictEqual(blankMessage.toString(), simpleStringMessage.toString(),
