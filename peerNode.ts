...
import BaseProvider from './storage_providers/baseProvider';
import { Block } from './types';
import { PeerCredentials } from './credentialProvider';
import Bundler from './bundler';
import Mempool from './models/Mempool';
import ConsensusEngine from './peerHandlers/ConsensusEngine';
import SyncEngine from './peerHandlers/SyncEngine';
import { ReputationManager } from './peerHandlers/ReputationManager';
    ...
    async init() {
        ...
        await new Promise<void>((resolve) => {
            httpServer.listen(this.port, '0.0.0.0', async () => {
                ...
                const originalOnClientMessage = (this.peer as any).onClientMessage.bind(this.peer);
                (this.peer as any).onClientMessage = async (connection: any, type: string, message: any) => {
