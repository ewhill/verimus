...
import { PendingBlockMessage } from '../messages/PendingBlockMessage';
import { VerifyBlockMessage } from '../messages/VerifyBlockMessage';
import { ProposeForkMessage } from '../messages/ProposeForkMessage';
import { AdoptForkMessage } from '../messages/AdoptForkMessage';
...
class ConsensusEngine {
    ...
    bindHandlers() {
        this.node.peer?.bind(PendingBlockMessage).to(async (m: PendingBlockMessage, c: PeerConnection) => this.handlePendingBlock(m.block, c, m.header?.timestamp.getTime()));
    ...
    async handlePendingBlock(block: Block, connection: PeerConnection, headerTimestamp: number) {
        ...
        const { verifySignature, signData } = require('../cryptoUtils');
