...
import { ChainStatusRequestMessage } from '../messages/ChainStatusRequestMessage';
import { ChainStatusResponseMessage } from '../messages/ChainStatusResponseMessage';
import { BlockSyncRequestMessage } from '../messages/BlockSyncRequestMessage';
import { BlockSyncResponseMessage } from '../messages/BlockSyncResponseMessage';
import { NetworkHealthSyncMessage } from '../messages/NetworkHealthSyncMessage';
...
class SyncEngine {
    ...
    bindHandlers() {
        ...
        if (this.node.peer) {
            this.syncInterval = setInterval(async () => {
                ...
                const score_payloads = peers.map(p => ({ publicKey: p.publicKey, score: p.score }));
    ...
    async handleNetworkHealthSync(score_payloads: { publicKey: string, score: number }[], connection: PeerConnection) {
        if (!this.node.ledger.peersCollection) return;
    ...
    async handleChainStatusRequest(connection: PeerConnection) {
        let pubKey = (connection as any).remoteCredentials_?.rsaKeyPair?.public?.toString('utf8');
        if (pubKey) {
            const now = Date.now();
    ...
    async performInitialSync() {
        ...
        if (highestConsensusIndex > localLatest.metadata.index) {
            ...
            for (let i = localLatest.metadata.index + 1; i <= highestConsensusIndex; i++) {
                ...
                let mainPubKey = (primaryHostConnection as any).remoteCredentials_?.rsaKeyPair?.public?.toString('utf8');
                if (mainPubKey) {
